---
title: "在Redhat Linux上PPTP VPN Server的安装配置"
date: "2009-09-21"
tags: ["Linux","PPTP"]
categories: ["DevOps"]
---
...
```
route add -net 192.168.5.0/24 dev eth1 #路由必须指定在内网网卡上，如上面的eth1
```

```
route add -net 192.168.5.0/24 gw 192.168.1.1 dev eth1 #需要网关进行跨网段连接的情况，如上面的192.168.1.1
```
...
```
iptables -t nat -A POSTROUTING -s 192.168.1.0/24 -o eth0 -j MASQUERADE #规则必须指定外网网卡与内网网段，如上面的eth0和内网网段192.168.1.0/24
```
